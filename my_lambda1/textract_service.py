...
def run_chunkr(bucket: str, key: str, file_id: str,region: str = "ap-south-1") -> Dict[str, Any]:
    """
    Distributed-safe Chunkr runner:
    - Uses Mongo locking (_id = fileId)
    - Reuses completed jobs when available
    - Extracts text using Chunkr instead of AWS Textract
    """
    temp_key = None
    ...
    try:
        ...
        if not claimed:
            ...
            if existing:
                ...
                print(f"ℹ️ Existing Chunkr job found for {file_id}: {status}")

                if status == "SUCCEEDED":
                    print("✅ Using cached Chunkr result from Mongo")
        ...
        print(f"✅ Downloaded temp file to {local_filepath}")

        # --- Mark job as started ---
        set_job_started(file_id, job_id="CHUNKR")
        ...
        print("✅ Chunkr job succeeded and stored in Mongo")

        return final_output  # ✅ MUST RETURN dict, not tuple

    except Exception as e:
        print(f"❌ run_chunkr error: {e}")
        traceback.print_exc()
